 ...
 *
 *  dplasma_zsyrk_pcctest_New - Generates the taskpool that performs the following operation
 *
 ...
 *          destroy with dplasma_zsyrk_pcctest_Destruct();
 *
 *******************************************************************************
 *
 ...
 ******************************************************************************/
parsec_taskpool_t*
dplasma_zsyrk_pcctest_New( dplasma_enum_t uplo,
 ...
 ******************************************************************************/
void
dplasma_zsyrk_pcctest_Destruct( parsec_taskpool_t *tp )
{
 ...
 * @ingroup dplasma_complex64
 *
 *  dplasma_zsyrk_pcctest - Performs the following operation
 *
 *    \f[ C = \alpha [ op( A ) \times op( A )' ] + \beta C \f],
 *
 *  where op( X ) is one of
 ...
 ******************************************************************************/
int
dplasma_zsyrk_pcctest( parsec_context_t *parsec,
               dplasma_enum_t uplo,
               dplasma_enum_t trans,
               dplasma_complex64_t alpha,
               const parsec_tiled_matrix_t *A,
               ...
               parsec_tiled_matrix_t *C)
{
    parsec_taskpool_t *parsec_zsyrk_pcctest = NULL;

    /* Check input arguments */
    if ((uplo != dplasmaLower) && (uplo != dplasmaUpper)) {
        dplasma_error("dplasma_zsyrk_pcctest", "illegal value of uplo");
        return -1;
    }
    if (trans != dplasmaTrans && trans != dplasmaNoTrans ) {
        dplasma_error("dplasma_zsyrk_pcctest", "illegal value of trans");
        return -2;
    }
    if ( (C->m != C->n) ) {
        dplasma_error("dplasma_zsyrk_pcctest", "illegal size of matrix C which should be square");
        return -6;
    }
    if ( ((trans == dplasmaNoTrans) && (A->m != C->m)) ||
         ((trans != dplasmaNoTrans) && (A->n != C->m)) ) {
        dplasma_error("dplasma_zsyrk_pcctest", "illegal size of matrix A");
        return -4;
    }

    parsec_zsyrk_pcctest = dplasma_zsyrk_pcctest_New(uplo, trans,
                                    alpha, A,
                                    beta, C);

    if ( parsec_zsyrk_pcctest != NULL )
    {
        parsec_context_add_taskpool( parsec, parsec_zsyrk_pcctest);
